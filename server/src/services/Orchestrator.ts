...
import { GameCommands, Timings } from "../../../shared/constants/constants.js";
...
import { GameEvents, PORT, RoomEvents } from "../constants/constants.js";
...
export default class Orchestrator {
  ...
  onConnection(socket: Socket) {
    ...
    if (client) {
      if (client.socket) {
        logger.log(`[Orchestrator] Duplicate connection attempt for client UUID: ${client.uuid}`);
   ...
   */
  #handleRoomErrors(fn: () => void, socket: Socket | null = null) {
    try {
      return fn();
    } catch (error) {
